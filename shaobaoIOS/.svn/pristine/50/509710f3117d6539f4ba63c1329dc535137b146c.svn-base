...
@interface MeetingInfoViewController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>
...
- (void)confirmBtnClicked
{
    ...
    [HTTP_MANAGER dealMeeting:self.m_info[@"roomId"]
                      address:self.m_info[@"roomAddress"]
                   relationId:self.m_info[@"relationId"]
                    auditType:@"3"
               successedBlock:^(NSDictionary *succeedResult) {
                   ...
                   {
                       [PubllicMaskViewHelper showTipViewWith:@"审核失败" inSuperView:self.view  withDuration:1];
                   }
...
- (void)cancelBtnClicked
{
    ...
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"确认不通过此次审核" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    [alert show];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [input setTextColor:UIColorFromRGB(0x797979)];
    ...
    if(indexPath.row == 0){
        ...
        [tip setText:@"会议布置要求:"];
        NSArray *arrMeetingType = [[self.m_info[@"meetingDemand"] stringByReplacingOccurrencesOfString:@" " withString:@""]componentsSeparatedByString:@","];
        BOOL is1 = [arrMeetingType containsObject:@"7"];
        BOOL is2 = [arrMeetingType containsObject:@"2"];
        BOOL is3 = [arrMeetingType containsObject:@"5"];
