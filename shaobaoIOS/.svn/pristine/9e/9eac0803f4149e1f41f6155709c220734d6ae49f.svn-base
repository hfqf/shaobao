                       ...
                       context:(void *)context
{
    if([keyPath isEqual:@"image"])
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    cutRect = CGRectMake(0,0,originalSize.height*rate,originalSize.height);
                }
                else
                {
                    cutRect = CGRectMake(0,(originalSize.height-high)/2,width,high);
                }
                CGImageRef cr = CGImageCreateWithImageInRect([newImage CGImage], cutRect);
                UIImage *cropped = [UIImage imageWithCGImage:cr];
                CGImageRelease(cr);
    ...
    {
        SpeLog(@"observeValueForKeyPath-->%@",keyPath);
    }
}
