...
#define kMinAlertHeight     280.0
#define kCancelButtonHeight   44.0
#define kCancelButtonWidth   180.0
...
+(MLTableAlert *)tableAlertWithTitle:(NSString *)title row:(NSInteger)row cancelButtonTitle:(NSString *)cancelBtnTitle  numberOfRows:(MLTableAlertNumberOfRowsBlock)rowsBlock andCells:(MLTableAlertTableCellsBlock)cellsBlock
{
	...
	return [[self alloc] initWithTitle:title row:(NSInteger)row cancelButtonTitle:cancelBtnTitle numberOfRows:rowsBlock andCells:cellsBlock];
}

#pragma mark - MLTableAlert Initialization

-(id)initWithTitle:(NSString *)title row:(NSInteger)row cancelButtonTitle:(NSString *)cancelButtonTitle numberOfRows:(MLTableAlertNumberOfRowsBlock)rowsBlock andCells:(MLTableAlertTableCellsBlock)cellsBlock
{
     m_row = row;
...
-(void)show
{
    ...
    [self.alertBg setBackgroundColor:[UIColor whiteColor]];
...
//	tableGradient.colors = [NSArray arrayWithObjects:(id)[[UIColor colorWithRed:240.0/255.0 green:240.0/255.0 blue:240.0/255.0 alpha:1.0] CGColor], (id)[[UIColor colorWithRed:174.0/255.0 green:174.0/255.0 blue:174.0/255.0 alpha:1.0] CGColor], nil];
	[self.table.backgroundView setBackgroundColor:[UIColor clearColor]];
...
//	[self.alertBg addSubview:maskShadow];
	...
	self.cancelButton.frame = CGRectMake(kLateralInset+(kTableAlertWidth- kCancelButtonWidth)/2, self.table.frame.origin.y + self.table.frame.size.height + kCancelButtonMargin, kCancelButtonWidth - kLateralInset * 2, kCancelButtonHeight);
	...
	self.alertBg.frame = CGRectMake((self.frame.size.width - kTableAlertWidth) / 2, (self.frame.size.height - self.height) / 2-20, kTableAlertWidth, self.height - kVerticalInset * 2+10);
    
    UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(0, 18, kTableAlertWidth, 20)];
    [lab setTextAlignment:NSTextAlignmentCenter];
    [lab setText:self.title];
    [lab setFont:[UIFont systemFontOfSize:20]];
    [lab setBackgroundColor:[UIColor clearColor]];
    [self.alertBg addSubview:lab];
    
    UIView *sep = [[UIView alloc]initWithFrame:CGRectMake(0,50, kTableAlertWidth,1)];
    [sep setBackgroundColor:[UIColor blackColor]];
    [self.alertBg addSubview:sep];
...
{
//    CGFloat hh = 160.0f + 38*(m_row - 1);
// 
//	if (hh < kMinAlertHeight)
//		_height = hh;
//	else
