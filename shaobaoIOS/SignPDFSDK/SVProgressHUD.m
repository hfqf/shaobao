...
@property (nonatomic, strong, readonly) UILabel *statusLable;
@property (nonatomic, strong, readonly) UIImageView *imageView;
@property (nonatomic, strong, readonly) UIActivityIndicatorView *spinnerView;//活动指示器
...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once, ^ {
        ...
        //NSLog(@"=====%f-------%f",sharedView.frame.origin.x,sharedView.frame.origin.y);
       // NSLog(@"=====%f-------%f",sharedView.bounds.size.width,sharedView.bounds.size.height);
    });
...
+ (void)showSuccessWithStatus:(NSString *)string {
    [SVProgressHUD showSuccessWithStatus:string duration:1 + string.length*0.3];
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.backgroundColor = [UIColor clearColor];//[UIColor colorWithPatternImage:[UIImage imageNamed:@"320x568.png"]];//320x568.png
		self.alpha = 1;
...
- (void)setStatus:(NSString *)string {
    ...
    //self.hudView.backgroundColor = [UIColor colorWithRed:135/255.0f green:135/255.0f blue:135/255.0f alpha:1];
    if(string) {
...
//        NSLog(@"stringSize:width = %f, height = %f",stringSize.width,stringSize.height);
//        NSLog(@"stringSize2:width = %f, height = %f",stringSize2.size.width, stringSize2.size.height);
...
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error {
	[self dismissWithStatus:string error:error afterDelay:1];
}
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.alpha != 1)
            return;
        if(error)
            self.imageView.image = [UIImage imageNamed:@"nav_back_1.png"];//SVProgressHUD.bundle/error.png
        else
            self.imageView.image = [UIImage imageNamed:@"success.png"];//SVProgressHUD.bundle/success.png
...
- (UIView *)hudView {
    if(!hudView) {
		...
		hudView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"320.png"]];
...
//        for (int i = 0; i < 8 ; i++) {
//            
//            NSString *str = [NSString stringWithFormat:@"run%d.png",i+1];
//            UIImage *image = [UIImage imageNamed:str];
//            [images addObject:image];
//        }
//        spinnerView = [[UIImageView alloc] init];
//        spinnerView.bounds = CGRectMake(0, 0, 55, 55);
//        //spinnerView.image = [UIImage imageNamed:@"run%d.png"];
//        spinnerView.animationDuration = 0.8;
//        spinnerView.animationImages = images;
...
+(void)setHidden:(BOOL)hidden
{
    [UIView animateWithDuration:0.5 animations:^{
        [[SVProgressHUD sharedView] setHidden:hidden];
    }];
}
