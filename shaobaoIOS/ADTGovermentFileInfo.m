...
+(ADTGovermentFileInfo *)getInfoFrom:(NSDictionary *)infoDic
{
    ...
    for(NSDictionary *cell in  arrFrom)
    {
        ...
        if([cell[@"fieldtype"]integerValue] < 80 || [cell[@"fieldtype"]integerValue] == 99)
        {
    ...
    for(NSDictionary *cell in  arrFrom)
    {
        ...
        NSArray *_arrCate = cell[@"catalogList"];
        NSMutableArray *_arrNewCate = [NSMutableArray array];
        for(NSDictionary *_infoCate in _arrCate){
            [_arrNewCate addObject:[NSMutableDictionary dictionaryWithDictionary:_infoCate]];
        }
        attachInfo.m_arrCatelogList = _arrNewCate;
