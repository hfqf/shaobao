...
//    [self setControlsHidden:YES animated:NO permanent:NO];
    ...
    if(_isPresent) self.view.backgroundColor = [UIColor blackColor];
...
//    self.navigationController.navigationBarHidden = YES;
    ...
    if(_isPresent) _pagingScrollView.backgroundColor = [UIColor blackColor];
...
/** 设置封面 */
- (void)setCoverBtnClicked
{
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"确定设置此图片为封面" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"好", nil];
    [alert show];
}

/** 评论 */
- (void)seeInfoBtnClicked
{
...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    [navBar setBackgroundColor:KEY_COMMON_CORLOR];
}
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageNamed:@"album_unsel"] forState:UIControlStateNormal];
                [selectedButton setImage:[UIImage imageNamed:@"album_sel"] forState:UIControlStateSelected];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
	...
	if (_currentPageIndex != previousCurrentPage) {
        if (!_defaultView) {
            title.text = [NSString stringWithFormat:@"%@ / %@", @(_currentPageIndex+1), @(self.m_arrItemData.count)];
        }
...
- (void)updateNavigation {
    ...
    /*
     // 设置标题
    title.hidden = NO;
    [title setTextColor:KEY_COMMON_CORLOR];
...
- (void)dismiss {
    ...
    if (self.isPresent) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation:)]) {
